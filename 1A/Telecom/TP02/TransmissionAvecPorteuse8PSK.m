...
dK = pskmod(mod, 2^ordreMod, pi/(2^ordreMod), 'gray');
figure;
plot(dK,'X');
...
zm = z(1:Ns:end);
moduleRecu = pskdemod(zm', 2^ordreMod, pi/(2^ordreMod), 'gray');
...
semilogy(SNRdBliste, TEB);
hold on;
semilogy(SNRdBliste, 2/ordreMod*qfunc(sqrt(2.0*ordreMod*10.^(SNRdBliste/10)) * sin(pi/(2^ordreMod))));
...
save('resultats8PSK.mat', 'TEB8PSK', 'DSP8PSK');