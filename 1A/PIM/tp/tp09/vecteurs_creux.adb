...
package body Vecteurs_Creux is
    ...
    begin
        if(V = null) then
            ...
            V.all.Valeur := Valeur;
            V.all.Suivant := null;
            --Put(Composante_Recursif(V, Indice),1); New_Line;
        else
            if (Composante_Recursif(V, Indice) = 0.0) then
                Vcopy := V;
                while(V.all.Suivant /= null and then V.all.Suivant.all.Indice <= Indice) loop
                    Vcopy := Vcopy.all.Suivant;
                end loop;
                Vi := new T_Cellule;
                Vi.all.Indice := Indice;
                Vi.all.Suivant := Vcopy.all.Suivant;
                Vi.all.Indice := Indice;
                Vi.all.Valeur := Valeur;
                Vcopy.all.Suivant := Vi;
            else
                Vcopy := V;
                while(V.all.Suivant /= null and then V.all.Suivant.all.Indice /= Indice) loop
                    Vcopy := Vcopy.all.Suivant;
                end loop;
                Vcopy.all.Suivant.all.Valeur := Valeur;
            end if;
        end if;
