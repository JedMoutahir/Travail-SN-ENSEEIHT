...
and ruleName env name =
  ...
  | Found (FrozenValue (fexpr, envf)) -> (value_of_expr fexpr envf)
  | Found value -> value
...
and ruleLet env ident bvalue bin =
  let value = (value_of_expr bvalue env) in match value with
    | (ErrorValue _) as result -> result
    | _ -> let nouvelEnv = (ident, value)::env in
      (value_of_expr bin nouvelEnv)
...
let fexprValue = value_of_expr fexpr env in (match fexprValue with
| FrozenValue (f, envf) -> (match f with
  | FunctionNode (param, corps) -> let evalenv = (param, FrozenValue (pexpr, env))::envf in
    value_of_expr corps evalenv
  | _ -> ErrorValue UndefinedExpressionError)
| ErrorValue _ as fexprValue -> fexprValue
| _ -> ErrorValue UndefinedExpressionError)
...
let pexprValue = value_of_expr pexpr env in 
 let fexprValue = value_of_expr fexpr env in (match fexprValue with
  | FrozenValue (f, envf) -> (match f with
    | FunctionNode (param, corps) -> let evalenv = (param, pexprValue)::envf in
      value_of_expr corps evalenv
    | _ -> ErrorValue UndefinedExpressionError)
  | ErrorValue _ as fexprValue -> fexprValue
  | _ -> ErrorValue UndefinedExpressionError)
...
and ruleLetrec env ident bvalue bin =
  let nouvelEnv = (ident, FrozenValue (LetrecNode(ident, bvalue, bvalue), env))::env in
    value_of_expr bin nouvelEnv
